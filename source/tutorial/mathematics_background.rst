...
In this section we will give a brief informal overview of some of the concepts of boundary integral equations.
...
As an intuitive explanation of these spaces consider that the space :math:`H^{0}(\Gamma)` is identical to the space of square integrable functions defined on :math:`\Gamma`. The space :math:`H^{1/2}(\Gamma)` is the correct space to represent the Dirichlet traces :math:`v` of the solution :math:`u` to the elliptic PDE :math:`Lu=0` in :math:`\Omega`, and the space :math:`H^{-1/2}(\Gamma)` is the natural space to represent the conormal derivatives :math:`t`. It follows that the Calderon projection acts on pairs of functions in the product space :math:`H^{1/2}(\Gamma)\times H^{-1/2}(\Gamma)`.
